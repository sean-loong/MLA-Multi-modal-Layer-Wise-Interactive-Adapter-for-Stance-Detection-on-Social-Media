...
parser.add_argument('--adapter_type', type=str, default='MMADAPTER')
GLOBAL_ARGS = parser.parse_args()


# ===== 新增日志路径自动生成逻辑 =====
trainData = 'in_target' if GLOBAL_ARGS.in_target else 'zero_shot'
...
with open(output_name, 'w') as f:
    f.write("Your training logs would go here.\n")
    # 或者你把后面的 print 替换成写入日志
print(f"Training started with logs saved to {output_name}")
# ==============================
...
# 固定MMA的参数config
if GLOBAL_ARGS.adapter_type == 'MMADAPTER':
...
def train_process(args, train_data_loader, valid_data_loader, test_data_loader, fold=0, target_name=None):
    ...
    adapter_identifiers = ['text_adapter', 'visual_adapter', 'shared_adapter']
    ...
    # 对参数分组后设置不同优化超参数，精细参数控制
        ...
        #  Adapter 参数（加 weight decay）
        {'params': [param for name, param in model.named_parameters()
                    if any(identifier in name for identifier in adapter_identifiers)
                    and not any(nd in name for nd in no_weight_decay_identifiers)],
        'lr': args.adapter_learning_rate,
        'weight_decay': args.weight_decay},
        {'params': [param for name, param in model.named_parameters()
                    if any(identifier in name for identifier in adapter_identifiers)
                    and any(nd in name for nd in no_weight_decay_identifiers)],
        'lr': args.adapter_learning_rate,
        'weight_decay': 0.0},
    ]
...
def main(args=None):
    if not GLOBAL_ARGS.debug_mode and not GLOBAL_ARGS.normal:
        # 如果不是 debug 模式也不是普通运行（normal）模式，就会使用 wandb 进行训练监控
    ...
    for target_name in target_names:
        # 对每个立场检测对象/人物（target_name）都进行了多轮训练（folds），得到了每个任务的多个结果。
        results = []
        acc = []
        f1 = []
        precision = []
...
if __name__ == '__main__':
    ...
    if GLOBAL_ARGS.debug_mode or GLOBAL_ARGS.normal:
        ...
        # AttributeDict 实际上是一个增强版字典，用点操作访问字段
        main(config)
